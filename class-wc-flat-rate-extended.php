...
class WC_Flat_Rate_Extended extends WC_Shipping_Method {
	/**
	...
	public function __construct() {
		$this->slug = 'woocommerce-flat-rate-extended';
		$this->id                 = 'flat_rate_ext'; // Id for your shipping method. Should be uunique.
		$this->method_title       = __( 'Flat Rate (Extended)', $this->slug );  // Title shown in admin
		$this->method_description = __( 'Fixed Rates, plus alternative rates for each shipping class.', $this->slug ); // Description shown in admin
		$this->flat_rate_option   = 'woocommerce_flat_rates_ext';
	...
	function item_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['quantity'] > 0 && $_product->needs_shipping() ) {
				...
				$fee = $cost = $cost_extra = 0;

				if ( isset( $this->flat_rates[ $shipping_class ] ) ) {
					$cost 			= $this->flat_rates[ $shipping_class ]['cost'];
					// check if has extra_cost: if false, set cost_extra = cost
					$cost_extra 	= isset( $this->flat_rates[ $shipping_class ]['cost_extra'] ) && $this->flat_rates[ $shipping_class ]['cost_extra'] != '' ? $this->flat_rates[ $shipping_class ]['cost_extra'] : $this->flat_rates[ $shipping_class ]['cost'];
					$fee			= $this->get_fee( $this->flat_rates[ $shipping_class ]['fee'], $_product->get_price() );
					$matched = true;
				} elseif ( $this->cost !== '' ) {
					$cost 			= $this->cost;
					// check if has extra_cost: if false, set cost_extra = cost
					$cost_extra 	= isset( $this->cost_extra ) && $this->cost_extra != '' ? $this->cost_extra : $this->cost;
				...
				<table class="shippingrows widefat" cellspacing="0">
					...
					<tbody class="flat_rates">
						<tr>
							...
							<td><input type="text" value="<?php echo esc_attr( wc_format_localized_price( $this->cost_extra ) ); ?>" name="default_cost_extra" placeholder="<?php _e( 'N/A', 'woocommerce' ); ?>" size="4" class="wc_input_price" /></td>
						...
						if ( $this->flat_rates ) {
							foreach ( $this->flat_rates as $class => $rate ) {
								...
								echo '</select>
									...
									<td><input type="text" value="' . esc_attr( $rate['cost_extra'] ) . '" name="' . esc_attr( $this->id .'_cost_extra[' . $i . ']' ) . '" placeholder="' . wc_format_localized_price( 0 ) . '" size="4" class="wc_input_price" /></td>
					...
					jQuery(function() {
						...
						jQuery('#<?php echo $this->id; ?>_flat_rates').on( 'click', 'a.add', function(){
							...
							jQuery('<tr class="flat_rate">\
								...
								<td><input type="text" name="<?php echo $this->id; ?>_cost_extra[' + size + ']" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" size="4" class="wc_input_price" /></td>\
	...
	function process_flat_rates() {
		...
		if ( isset( $_POST[ $this->id . '_cost_extra'] ) )  $flat_rate_cost_extra  = array_map( 'stripslashes', $_POST[ $this->id . '_cost_extra'] );
		...
		for ( $i = 0; $i <= $key; $i++ ) {
			if ( ! empty( $flat_rate_class[ $i ] ) && isset( $flat_rate_cost[ $i ] ) && isset( $flat_rate_fee[ $i ] ) ) {
				...
				$flat_rate_cost_extra[ $i ] = wc_format_decimal( $flat_rate_cost_extra[$i] );
				...
				$flat_rates[ sanitize_title($flat_rate_class[$i]) ] = array(
					...
					'cost_extra' => $flat_rate_cost_extra[ $i ],
					'fee'  => $flat_rate_fee[ $i ],
				);
			}
		}
	...
	function save_default_costs( $fields ) {
	 	...
	 	$default_cost_extra = ( $_POST['default_cost_extra'] === '' ) ? '' : wc_format_decimal( $_POST['default_cost_extra'] );
	 	...
	 	$fields['cost_extra'] = $default_cost_extra;
